...
import com.onewhohears.dscombat.client.sounds.DistantGunfireSoundInstance;
import com.onewhohears.dscombat.client.sounds.DopplerSoundInstance;
import com.onewhohears.dscombat.client.sounds.MissileEngineSoundInstance;
...
public class UtilClientSafeSounds {
	...
	public static void dopplerSound(Entity entity, SoundEvent sound, 
			float initVolume, float initPitch, float velSound, boolean delayed, double range) {
		Minecraft m = Minecraft.getInstance();
		LocalPlayer p = m.player;
		if (p == null) return;
		int delay = 0;
		if (delayed) delay = (int)(p.distanceTo(entity) / velSound);
		m.getSoundManager().playDelayed(new DopplerSoundInstance(sound, 
				p, entity, initVolume, initPitch, velSound, range), delay);
	}
	
	public static void missileEngineSound(Entity entity, SoundEvent sound, 
			float baseVolume, float basePitch) {
		Minecraft m = Minecraft.getInstance();
		LocalPlayer p = m.player;
		if (p == null) return;
		m.getSoundManager().play(new MissileEngineSoundInstance(sound, 
				p, entity, baseVolume, basePitch));
	}
	...
	public static void aircraftRadio(EntityVehicle plane, String sound) {
		try {
			SoundEvent se = UtilClientPacket.getSoundByIdClient(sound, SoundEvents.MUSIC_DISC_BLOCKS);
	...
	/**
	 * Plays a distant gunfire/cannon sound that can be heard from far away.
	 * Creates a "cannonade" effect for battlefield ambience.
	 * 
	 * @param sourcePos Position where the shot was fired
	 * @param sound The distant gunfire sound to play
	 * @param baseVolume Base volume (will be attenuated by distance)
	 * @param basePitch Base pitch (will be slightly lowered for distance)
	 * @param durationTicks How long the sound should play (in ticks)
	 */
	public static void distantGunfireSound(Vec3 sourcePos, SoundEvent sound, 
			float baseVolume, float basePitch, int durationTicks) {
		Minecraft m = Minecraft.getInstance();
		LocalPlayer p = m.player;
		if (p == null) return;
		
		// Calculate delay based on distance (sound travels at speed of sound)
		// Using a faster speed for better gameplay feel (343 m/s = ~17 blocks/tick at normal scale)
		// We use a fixed speed to avoid very long delays with low IRL scales
		float distance = (float) p.position().distanceTo(sourcePos);
		float soundSpeed = 17.0f; // blocks per tick (343 m/s in real life, adjusted for gameplay)
		int delay = (int)(distance / soundSpeed);
		
		// Cap maximum delay to 2 seconds (40 ticks) for better gameplay
		delay = Math.min(delay, 40);
		
		m.getSoundManager().playDelayed(new DistantGunfireSoundInstance(sound, 
				p, sourcePos, baseVolume, basePitch, durationTicks), delay);
	}
	
	public static void vehicleIdleSound(EntityVehicle vehicle, SoundEvent nonPassengerIdle, 
	                                     SoundEvent passengerIdle) {
		Minecraft m = Minecraft.getInstance();
		LocalPlayer player = m.player;
		if (player == null) return;
		
		boolean isPassenger = vehicle.isVehicleOf(player);
		
		// Play non-passenger idle sound with fade in
		if (nonPassengerIdle != null && !isPassenger) {
			com.onewhohears.dscombat.client.sounds.VehicleIdleSoundInstance sound = 
				new com.onewhohears.dscombat.client.sounds.VehicleIdleSoundInstance(
					nonPassengerIdle, player, vehicle, false, 0.5f
				);
			m.getSoundManager().play(sound);
		}
		
		// Play passenger idle sound with fade in
		if (passengerIdle != null && isPassenger) {
			com.onewhohears.dscombat.client.sounds.VehicleIdleSoundInstance sound = 
				new com.onewhohears.dscombat.client.sounds.VehicleIdleSoundInstance(
					passengerIdle, player, vehicle, true, 0.3f
				);
			m.getSoundManager().play(sound);
		}
	}
	
	public static void registerDynamicTexture(ResourceLocation textLoc, 
	                                          net.minecraft.client.renderer.texture.DynamicTexture texture) {
		Minecraft.getInstance().getTextureManager().register(textLoc, texture);
	}
