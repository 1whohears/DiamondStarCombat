(function () {
    ...
    // majority of this code was shamelessly stolen from https://github.com/JannisX11/blockbench/blob/master/js/io/formats/obj.js
	// .obj files can use the 'g' command to group 'o's into folders
	...
	function getGroupNames(element) {
		let names = "";
		let par = element.parent;
		while (par.name != undefined && par.name != "root") {
			names = names.concat(par.name+" ");
			par = par.parent;
			if (par == undefined) {
				break;
			}
	...
	function addGroups(output, element) {
		if (element.parent.name != undefined && element.parent.name != "root") {
			output.push('g '+getGroupNames(element));
		}
	}
    ...
    var codec = new Codec('obj', {
        ...
        write(content, path, mtlMode) {
            var scope = this;

            var mtl_path = path.replace(/\.obj$/, '.mtl')
                content = this.compile(mtlMode, {
                    mtl_name: pathToName(mtl_path, true)
                })
                Blockbench.writeFile(path, {
                    content
                }, path => scope.afterSave(path));

            Blockbench.writeFile(mtl_path, {
                content: _obj_export.mtl
            });
        ...
        export(mtlMode) {
            ...
            if (isApp) {
                Blockbench.export({
                    ...
                    name: this.fileName(),
                    custom_writer: (a, b) => scope.write(a, b, mtlMode),
                })
            } else {
                var archive = new JSZip();
                var content = this.compile(mtlMode)
                    ...
                    for (var key in _obj_export.images) {
                        var texture = _obj_export.images[key]
                            if (texture && !texture.error && texture.mode === 'bitmap') {
                                archive.file(pathToName(texture.name) + '.png', texture.source.replace('data:image/png;base64,', ''), {
                                    base64: true
                                });
                            }
                    }
		...
		var exportGroupObjButton;
		var exportDSCVehicleButton;
        ...
        Plugin.register('export_obj_groups', {
            ...
            variant: 'both',
            onload() {
				exportGroupObjButton = new Action('export_obj_with_groups', {
					name: 'Export OBJ with Groups',
				...
				exportDSCVehicleButton = new Action('export_dsc_vehicle_obj', {
					name: 'Export DSC Vehicle OBJ Model',
					description: 'Same as Export with Groups, but the mtl file is correctly formatted.',
					icon: 'icon-objects',
					click: function() {
						codec.export('dsc_vehicle')
					}
				});
				MenuBar.addAction(exportGroupObjButton, "file.export");
				MenuBar.addAction(exportDSCVehicleButton, "file.export");
            },
            onunload() {
				exportGroupObjButton.delete();
				exportDSCVehicleButton.delete();
			}
        });
