...
public class DriveNodeEvaluator extends VehicleNodeEvaluator {
    ...
    public void prepare(PathNavigationRegion pathNavigationRegion, EntityVehicle vehicle) {
        super.prepare(pathNavigationRegion, vehicle);
        //vehicle.onPathfindingStart();
    }

    public void done() {
        //this.vehicle.onPathfindingDone();
    ...
    public Node getStart() {
        ...
        BlockState blockState = this.level.getBlockState(mutableBlockPos.set(this.vehicle.getX(), (double)i, this.vehicle.getZ()));
        if (!this.vehicle.canStandOnFluid(blockState.getFluidState())) {
            if (this.canFloat() && this.vehicle.isInWater()) {
                while(true) {
                    ...
                    ++i;
                    blockState = this.level.getBlockState(mutableBlockPos.set(this.vehicle.getX(), (double)i, this.vehicle.getZ()));
                }
            } else if (this.vehicle.onGround()) {
                i = Mth.floor(this.vehicle.getY() + (double)0.5F);
            } else {
                BlockPos blockPos;
            ...
            while(this.vehicle.canStandOnFluid(blockState.getFluidState())) {
                ++i;
                blockState = this.level.getBlockState(mutableBlockPos.set(this.vehicle.getX(), (double)i, this.vehicle.getZ()));
            }
        ...
        BlockPos blockPos = this.vehicle.blockPosition();
    ...
    protected Node getStartNode(BlockPos blockPos) {
        ...
        node.type = this.getBlockPathType(this.vehicle, node.asBlockPos());
        node.costMalus = this.vehicle.getPathfindingMalus(node.type);
        return node;
    }

    protected boolean canStartAt(BlockPos blockPos) {
        BlockPathTypes blockPathTypes = this.getBlockPathType(this.vehicle, blockPos);
        return blockPathTypes != BlockPathTypes.OPEN && this.vehicle.getPathfindingMalus(blockPathTypes) >= 0.0F;
    }
    ...
    public int getNeighbors(Node[] nodes, Node node) {
        int i = 0;
        int j = 0;
        BlockPathTypes blockPathTypes = this.getCachedBlockType(this.vehicle, node.x, node.y + 1, node.z);
        BlockPathTypes blockPathTypes2 = this.getCachedBlockType(this.vehicle, node.x, node.y, node.z);
        if (this.vehicle.getPathfindingMalus(blockPathTypes) >= 0.0F && blockPathTypes2 != BlockPathTypes.STICKY_HONEY) {
            j = Mth.floor(Math.max(1.0F, this.vehicle.maxUpStep()));
        }
        ...
        if (node4 != null && node3 != null && node2 != null) {
            if (node4.closed) {
                ...
                if (node2.type != BlockPathTypes.WALKABLE_DOOR && node3.type != BlockPathTypes.WALKABLE_DOOR && node4.type != BlockPathTypes.WALKABLE_DOOR) {
                    boolean bl = node3.type == BlockPathTypes.FENCE && node2.type == BlockPathTypes.FENCE && (double)this.vehicle.getBbWidth() < (double)0.5F;
    ...
    private boolean canReachWithoutCollision(Node node) {
        AABB aABB = this.vehicle.getBoundingBox();
        Vec3 vec3 = new Vec3((double)node.x - this.vehicle.getX() + aABB.getXsize() / (double)2.0F, (double)node.y - this.vehicle.getY() + aABB.getYsize() / (double)2.0F, (double)node.z - this.vehicle.getZ() + aABB.getZsize() / (double)2.0F);
    ...
    protected double getFloorLevel(BlockPos blockPos) {
        return (this.canFloat() || this.isAmphibious()) && this.level.getFluidState(blockPos).is(FluidTags.WATER) ? (double)blockPos.getY() + (double)0.5F : getFloorLevel(this.level, blockPos);
    }
    ...
    public static double getFloorLevel(BlockGetter blockGetter, BlockPos blockPos) {
        ...
        return (double)blockPos2.getY() + (voxelShape.isEmpty() ? (double)0.0F : voxelShape.max(Direction.Axis.Y));
    }
    ...
    protected Node findAcceptedNode(int i, int j, int k, int l, double d, Direction direction, BlockPathTypes blockPathTypes) {
        ...
        if (e - d > this.getMobJumpHeight()) {
            ...
            BlockPathTypes blockPathTypes2 = this.getCachedBlockType(this.vehicle, i, j, k);
            float f = this.vehicle.getPathfindingMalus(blockPathTypes2);
            double g = (double)this.vehicle.getBbWidth() / (double)2.0F;
            if (f >= 0.0F) {
            ...
            if (blockPathTypes2 != BlockPathTypes.WALKABLE && (!this.isAmphibious() || blockPathTypes2 != BlockPathTypes.WATER)) {
                if ((node == null || node.costMalus < 0.0F) && l > 0 && (blockPathTypes2 != BlockPathTypes.FENCE || this.canWalkOverFences()) && blockPathTypes2 != BlockPathTypes.UNPASSABLE_RAIL && blockPathTypes2 != BlockPathTypes.TRAPDOOR && blockPathTypes2 != BlockPathTypes.POWDER_SNOW) {
                    ...
                    if (node != null && (node.type == BlockPathTypes.OPEN || node.type == BlockPathTypes.WALKABLE) && this.vehicle.getBbWidth() < 1.0F) {
                        double h = (double)(i - direction.getStepX()) + (double)0.5F;
                        double m = (double)(k - direction.getStepZ()) + (double)0.5F;
                        AABB aABB = new AABB(h - g, this.getFloorLevel(mutableBlockPos.set(h, (double)(j + 1), m)) + 0.001, m - g, h + g, (double)this.vehicle.getBbHeight() + this.getFloorLevel(mutableBlockPos.set((double)node.x, (double)node.y, (double)node.z)) - 0.002, m + g);
                        if (this.hasCollisions(aABB)) {
                            node = null;
                        }
                    }
                ...
                if (!this.isAmphibious() && blockPathTypes2 == BlockPathTypes.WATER && !this.canFloat()) {
                    if (this.getCachedBlockType(this.vehicle, i, j - 1, k) != BlockPathTypes.WATER) {
                        return node;
                    }

                    while(j > this.vehicle.level().getMinBuildHeight()) {
                        --j;
                        blockPathTypes2 = this.getCachedBlockType(this.vehicle, i, j, k);
                        if (blockPathTypes2 != BlockPathTypes.WATER) {
                            return node;
                        }

                        node = this.getNodeAndUpdateCostToMax(i, j, k, blockPathTypes2, this.vehicle.getPathfindingMalus(blockPathTypes2));
                    }
                }
                ...
                if (blockPathTypes2 == BlockPathTypes.OPEN) {
                    ...
                    while(blockPathTypes2 == BlockPathTypes.OPEN) {
                        --j;
                        if (j < this.vehicle.level().getMinBuildHeight()) {
                            return this.getBlockedNode(i, o, k);
                        }

                        if (n++ >= this.vehicle.getMaxFallDistance()) {
                            return this.getBlockedNode(i, j, k);
                        }

                        blockPathTypes2 = this.getCachedBlockType(this.vehicle, i, j, k);
                        f = this.vehicle.getPathfindingMalus(blockPathTypes2);
    ...
    private double getMobJumpHeight() {
        return Math.max((double)1.125F, (double)this.vehicle.maxUpStep());
    }
    ...
    private boolean hasCollisions(AABB aABB) {
        return this.collisionCache.computeIfAbsent(aABB, (object) -> !this.level.noCollision(this.vehicle, aABB));
    }

    public BlockPathTypes getBlockPathType(BlockGetter blockGetter, int i, int j, int k, EntityVehicle vehicle) {
        EnumSet<BlockPathTypes> enumSet = EnumSet.noneOf(BlockPathTypes.class);
        BlockPathTypes blockPathTypes = BlockPathTypes.BLOCKED;
        blockPathTypes = this.getBlockPathTypes(blockGetter, i, j, k, enumSet, blockPathTypes, vehicle.blockPosition());
        if (enumSet.contains(BlockPathTypes.FENCE)) {
            ...
            for(BlockPathTypes blockPathTypes3 : enumSet) {
                if (vehicle.getPathfindingMalus(blockPathTypes3) < 0.0F) {
                    return blockPathTypes3;
                }

                if (vehicle.getPathfindingMalus(blockPathTypes3) >= vehicle.getPathfindingMalus(blockPathTypes2)) {
                    blockPathTypes2 = blockPathTypes3;
                }
            }

            if (blockPathTypes == BlockPathTypes.OPEN && vehicle.getPathfindingMalus(blockPathTypes2) == 0.0F && this.entityWidth <= 1) {
    ...
    protected BlockPathTypes getBlockPathType(EntityVehicle vehicle, BlockPos blockPos) {
        return this.getCachedBlockType(vehicle, blockPos.getX(), blockPos.getY(), blockPos.getZ());
    }

    protected BlockPathTypes getCachedBlockType(EntityVehicle vehicle, int i, int j, int k) {
        return (BlockPathTypes)this.pathTypesByPosCache.computeIfAbsent(BlockPos.asLong(i, j, k), (l) -> this.getBlockPathType(this.level, i, j, k, vehicle));
    }
