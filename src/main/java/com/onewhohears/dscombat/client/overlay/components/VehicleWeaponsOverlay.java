...
public class VehicleWeaponsOverlay extends VehicleOverlayComponent {
        ...
        if (!this.weaponChangeState) {
            ...
            for (int i = 0; i < weaponTabsToRender; i++) {
                ...
                if (!weaponAt.getStats().isNoWeapon()) {
                    drawString(poseStack, getFont(), weaponAt.getCurrentAmmo() + "/" + weaponAt.getStats().getMaxAmmo(), 16, newYPos + 14, 0xe6e600);
                } else {
                    drawString(poseStack, getFont(), SAFETY, 16, newYPos + 14, 0xff5555);
                }
                drawString(poseStack, getFont(), weaponAt.getStats().getDisplayNameComponent(), 16, newYPos + 4, 0xffffff);
                poseStack.popPose();
            }
    ...
    private static void drawWeapon(PoseStack poseStack, WeaponInstance<?> selectedWeapon, double yPlacement, int blitPosition) {
    	if (selectedWeapon == null) return;
    	WeaponTabComponent.drawWeaponName(poseStack, selectedWeapon.getStats().getDisplayNameComponent(), 13, yPlacement, blitPosition - 2);
        ...
        if (!selectedWeapon.getStats().isNoWeapon()) drawString(poseStack, getFont(), selectedWeapon.getCurrentAmmo() + "/" + selectedWeapon.getStats().getMaxAmmo(), 16, (int) (yPlacement + 14), 0xe6e600);
        drawString(poseStack, getFont(), selectedWeapon.getStats().getWeaponTypeCode(), 16, (int) yPlacement + 4, 0xe6e600);
        poseStack.popPose();
    }
}
