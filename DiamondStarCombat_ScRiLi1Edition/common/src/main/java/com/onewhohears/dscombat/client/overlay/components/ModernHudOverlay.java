...
public class ModernHudOverlay extends VehicleOverlayComponent {
    ...
    private void drawEditHandle(GuiGraphics graphics, int x, int y, int w, int h, String label, int text) {
        int border = 1;
        graphics.fill(x - border, y - border, x + w + border, y + h + border, withAlpha(new Color(255,255,255).getRGB(), 0.12f));
        graphics.drawString(FONT, label, x + 2, y - 10, text);
        // resize handle (triangle at bottom-right)
        graphics.fill(x + w - 8, y + h - 2, x + w + 2, y + h + 2, withAlpha(new Color(255,255,255).getRGB(), 0.3f));
    }
    ...
    private void drawHintsCompact(GuiGraphics graphics, int sw, int sh, int text) {
        ...
        int y = sh - 14;
        graphics.drawCenteredString(FONT, s, sw / 2, y, text);
    }
    ...
    private void drawHintsFull(GuiGraphics graphics, int sw, int sh, int text) {
        ...
        graphics.fill(x, y, x + w, y + h, withAlpha(new Color(20,20,20).getRGB(), 0.85f));
        int yy = y + 8;
        graphics.drawString(FONT, "Controls (F8 to hide)", x + 8, yy, text); yy += 12;
        // List a curated set of important bindings
        graphics.drawString(FONT, "Hover: Space", x + 8, yy, text); yy += 10;
        graphics.drawString(FONT, "Gear: K", x + 8, yy, text); yy += 10;
        graphics.drawString(FONT, "Flares: V", x + 8, yy, text); yy += 10;
        graphics.drawString(FONT, "Radar Mode: O", x + 8, yy, text); yy += 10;
        graphics.drawString(FONT, "Track Target: RSHIFT", x + 8, yy, text); yy += 10;
        graphics.drawString(FONT, "Gimbal: ;", x + 8, yy, text); yy += 10;
        graphics.drawString(FONT, "Throttle: Up/Down", x + 8, yy, text); yy += 10;
        graphics.drawString(FONT, "Yaw: A/D", x + 8, yy, text); yy += 10;
        graphics.drawString(FONT, "Roll: Left/Right", x + 8, yy, text); yy += 10;
        graphics.drawString(FONT, "Pitch: W/S", x + 8, yy, text); yy += 10;
        graphics.drawString(FONT, "HUD Mode: F7", x + 8, yy, text); yy += 10;
    }
    ...
    protected void render(Gui gui, GuiGraphics graphics, float partialTick, int screenWidth, int screenHeight) {
        ...
        if (localHudMode == 1) {
            graphics.fill(attX, attY, attX + attW, attY + attH, withAlpha(new Color(16, 16, 16).getRGB(), alpha * 0.6f));
        }
        ...
        if (localHudMode == 1) graphics.fill(hrX, hrY, hrX + hrW, hrY + hrH, withAlpha(new Color(16, 16, 16).getRGB(), alpha * 0.6f));
        drawHeadingRibbon(graphics, v, hrX, hrY, hrW, hrH, text, accent);

        // Draw attitude ladder (simple horizon with pitch offset and bank rotation)
        drawAttitude(graphics, v, attX, attY, attW, attH, text, accent);

        // Plane-only: show AOA near attitude
        if (isPlane) {
            try {
                double aoa = ((EntityPlane) v).getAOA();
                graphics.drawString(FONT, String.format("AOA %3.1f", aoa), attX + attW + 6, attY + attH/2 - 4, text);
        ...
        if (localHudMode == 1) graphics.fill(prX, prY, prX + prW, prY + prH, panel);
        ...
        int barH = prH - 20;
        // Track background
        if (localHudMode == 1) graphics.fill(barX, barY, barX + barW, barY + barH, withAlpha(new Color(60, 60, 60).getRGB(), alpha));
        // Fill
        int fillH = (int) (barH * Math.max(0f, Math.min(1f, throttlePct)));
        if (localHudMode == 1) {
            graphics.fill(barX, barY + (barH - fillH), barX + barW, barY + barH, withAlpha(accent, alpha));
            graphics.drawString(FONT, "THR", barX - 2, prY + prH - 12, text);
        }
        ...
        int rbW = barW;
        int rbH = barH;
        if (localHudMode == 1 && isHeli) graphics.fill(rbX, rbY, rbX + rbW, rbY + rbH, withAlpha(new Color(60, 60, 60).getRGB(), alpha));
        int rpmFill = (int) (rbH * rpmPct);
        if (localHudMode == 1 && isHeli) {
            graphics.fill(rbX, rbY + (rbH - rpmFill), rbX + rbW, rbY + rbH, withAlpha(warn, alpha));
            graphics.drawString(FONT, "RPM", rbX - 2, prY + prH - 12, text);
        }
        ...
        graphics.drawString(FONT, String.format("spd  %4.1f %s", speed, speedUnitLabel()), metricsX, metricsY, text);
        if (localHudMode == 1) graphics.drawString(FONT, String.format("clb  %4.1f %s", climb, speedUnitLabel()), metricsX, metricsY + 10, climb >= 0 ? text : warn);
        graphics.drawString(FONT, String.format("hdg  %3.0f°", (v.getYRot() + 360f) % 360f), metricsX, metricsY + (localHudMode == 1 ? 20 : 10), text);
        ...
        int bw = 120, bh = 6;
        if (localHudMode == 1) {
            graphics.fill(barsX, barsY, barsX + bw, barsY + bh, withAlpha(new Color(60,60,60).getRGB(), alpha));
            graphics.fill(barsX, barsY, barsX + (int)(bw * fuelPct), barsY + bh, withAlpha(new Color(0, 180, 255).getRGB(), alpha));
            graphics.drawString(FONT, String.format("fuel %3.0f%%", fuelPct * 100f), barsX + bw + 6, barsY - 1, text);
        }
        // Health
        barsY += bh + 4;
        ...
        if (localHudMode == 1) {
            graphics.fill(barsX, barsY, barsX + bw, barsY + bh, withAlpha(new Color(60,60,60).getRGB(), alpha));
            graphics.fill(barsX, barsY, barsX + (int)(bw * hpPct), barsY + bh, withAlpha(new Color(80, 220, 80).getRGB(), alpha));
            graphics.drawString(FONT, String.format("hp %3.0f%%", hpPct * 100f), barsX + bw + 6, barsY - 1, text);
        }
        // Armor
        barsY += bh + 4;
        ...
        if (localHudMode == 1) {
            graphics.fill(barsX, barsY, barsX + bw, barsY + bh, withAlpha(new Color(60,60,60).getRGB(), alpha));
            graphics.fill(barsX, barsY, barsX + (int)(bw * arPct), barsY + bh, withAlpha(new Color(180, 180, 60).getRGB(), alpha));
            graphics.drawString(FONT, String.format("arm %3.0f%%", arPct * 100f), barsX + bw + 6, barsY - 1, text);
        }
        ...
        graphics.fill(plX, plY, plX + plW, plY + plH, withAlpha(new Color(24,24,24).getRGB(), alpha * (localHudMode == 0 ? 0.5f : 0.85f)));
        ...
        int tapeW = 14;
        int tapeH = plH - 20;
        graphics.fill(tapeX, tapeY, tapeX + tapeW, tapeY + tapeH, withAlpha(new Color(60,60,60).getRGB(), alpha));
        double alt = altConverted(v.getAltitude());
        graphics.drawString(FONT, String.format("alt %4.0f %s", alt, altUnitLabel()), tapeX + tapeW + 8, tapeY, text);
        // Pointer
        int pty = tapeY + tapeH / 2;
        graphics.fill(tapeX + tapeW + 2, pty - 1, tapeX + tapeW + 10, pty + 1, withAlpha(accent, alpha));
        ...
        if (isHeli) {
            ...
            graphics.fill(plX + 10, chipY, plX + 10 + chipW, chipY + chipH, c1);
            graphics.drawString(FONT, "IGE", plX + 12, chipY + 3, Color.BLACK.getRGB());
            ...
            graphics.fill(plX + 10 + chipW + 6, chipY, plX + 10 + chipW * 2 + 6, chipY + chipH, c2);
            graphics.drawString(FONT, "ETL", plX + 12 + chipW + 6, chipY + 3, Color.BLACK.getRGB());
            ...
            graphics.fill(plX + 10 + (chipW + 6) * 2, chipY, plX + 10 + (chipW + 6) * 3, chipY + chipH, c3);
            graphics.drawString(FONT, "VRS", plX + 12 + (chipW + 6) * 2, chipY + 3, Color.BLACK.getRGB());
        } else if (isPlane) {
            // Plane: GEAR / FLAPS
            ...
            graphics.fill(plX + 10, chipY, plX + 10 + chipW, chipY + chipH, cg);
            graphics.drawString(FONT, "GEAR", plX + 12, chipY + 3, Color.BLACK.getRGB());

            boolean flaps = v.isFlapsDown();
            int cf = withAlpha(new Color(flaps ? 220 : 60, flaps ? 180 : 60, 88).getRGB(), chipAlpha);
            graphics.fill(plX + 10 + chipW + 6, chipY, plX + 10 + chipW * 2 + 6, chipY + chipH, cf);
            graphics.drawString(FONT, "FLAPS", plX + 12 + chipW + 6, chipY + 3, Color.BLACK.getRGB());
        }
        ...
        if (localHudMode == 1) graphics.fill(stX, stY, stX + stW, stY + stH, withAlpha(new Color(16, 16, 16).getRGB(), alpha * 0.6f));
        double spd = speedConverted(v.getDeltaMovement().length() * 20.0);
        if (localHudMode == 1) graphics.drawString(FONT, String.format("%4.1f %s", spd, speedUnitLabel()), stX + 4, stY + stH/2 - 4, text);
        ...
        if (localHudMode == 1) {
            graphics.fill(atX, atY, atX + atW, atY + atH, withAlpha(new Color(16, 16, 16).getRGB(), alpha * 0.6f));
            graphics.drawString(FONT, String.format("%4.0f %s", alt, altUnitLabel()), atX + 4, atY + atH/2 - 4, text);
        }

        // Compact minimal numeric (top corners) to reduce clutter
        if (localHudMode == 0) {
            graphics.drawString(FONT, String.format("%4.1f %s", speed, speedUnitLabel()), 12, 12, text);
            graphics.drawString(FONT, String.format("%4.0f %s", alt, altUnitLabel()), sw - 90, 12, text);
        }

        // Draw compact key-hints row or full legacy-like list on demand
        if (localHintsMode == 2) drawHintsFull(graphics, sw, sh, text);
        else if (localHintsMode == 1) drawHintsCompact(graphics, sw, sh, text);

        // Layout edit overlay and interaction
        if (editMode) {
            drawEditHandle(graphics, attX, attY, attW, attH, "att", text);
            drawEditHandle(graphics, hrX, hrY, hrW, hrH, "head", text);
            drawEditHandle(graphics, prX, prY, prW, prH, "br", text);
            drawEditHandle(graphics, plX, plY, plW, plH, "bl", text);
    ...
    private void drawHeadingRibbon(GuiGraphics graphics, EntityVehicle v, int x, int y, int w, int h, int text, int accent) {
        ...
        // center caret
        graphics.fill(x + w/2 - 1, y, x + w/2 + 1, y + h, accent);
        ...
        for (int deg = start - (start % 10); deg <= start + spanDeg; deg += 10) {
            ...
            graphics.fill(x + d, y + 2, x + d + 1, y + vlen, withAlpha(text, 0.5f));
            if (deg % 30 == 0) {
                int label = ((deg % 360) + 360) % 360;
                String s = Integer.toString(label);
                graphics.drawCenteredString(FONT, s, x + d, y + h - 12, text);
            }
        }
    }
    ...
    private void drawAttitude(GuiGraphics graphics, EntityVehicle v, int x, int y, int w, int h, int text, int accent) {
        ...
        graphics.pose().mulPose(Vec3f.ZP.rotationDegrees(-roll).convert());
        int bandW = w - 10;
        int cy = (int)(pitch * pxPerDeg);
        // horizon line
        graphics.fill(-bandW/2, cy - 1, bandW/2, cy + 1, text);
        ...
        for (int pd = -45; pd <= 45; pd += 15) {
            ...
            graphics.fill(-len, yy, -2, yy + 1, withAlpha(text, 0.6f));
            graphics.fill(2, yy, len, yy + 1, withAlpha(text, 0.6f));
        }
        // center caret
        graphics.fill(-6, -1, 6, 1, accent);
        graphics.fill(-1, -6, 1, 6, accent);
        graphics.pose().popPose();
    }
