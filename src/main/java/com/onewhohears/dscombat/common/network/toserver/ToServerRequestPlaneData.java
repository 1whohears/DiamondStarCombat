		...
		ctx.get().enqueueWork(() -> {
			...
			if (level.getEntity(id) instanceof EntityAircraft plane) {
				PacketHandler.INSTANCE.send(PacketDistributor.TRACKING_ENTITY.with(() -> plane), 
					new ToClientRecievePlaneData(plane.getId(), plane.weaponSystem.getSelectedIndex(),
						plane.weaponSystem.getWeapons(), plane.partsManager.getSlots(), plane.radarSystem.getRadars()));
			}
			success.set(true);
		});
