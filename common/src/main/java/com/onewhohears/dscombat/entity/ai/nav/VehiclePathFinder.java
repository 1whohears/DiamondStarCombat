...
public class VehiclePathFinder {
    ...
    private final VehicleNodeEvaluator nodeEvaluator;
    private static final boolean DEBUG = false;
    private final BinaryHeap openSet = new BinaryHeap();

    public VehiclePathFinder(VehicleNodeEvaluator nodeEvaluator, int i) {
        this.nodeEvaluator = nodeEvaluator;
        this.maxVisitedNodes = i;
    }

    @Nullable
    public Path findPath(PathNavigationRegion pathNavigationRegion, EntityVehicle vehicle, Set<BlockPos> set, float f, int i, float g) {
        this.openSet.clear();
        this.nodeEvaluator.prepare(pathNavigationRegion, vehicle);
        Node node = this.nodeEvaluator.getStart();
        if (node == null) {
            return null;
        } else {
            Map<Target, BlockPos> map = set.stream().collect(Collectors.toMap((blockPos) ->
                    this.nodeEvaluator.getGoal(blockPos.getX(), blockPos.getY(), blockPos.getZ()), Function.identity()));
    ...
    private Path findPath(ProfilerFiller profilerFiller, Node node, Map<Target, BlockPos> map, float f, int i, float g) {
        ...
        Optional<Path> optional = !set3.isEmpty() ? set3.stream().map((targetx) -> this.reconstructPath(targetx.getBestNode(), (BlockPos)map.get(targetx), true)).min(Comparator.comparingInt(Path::getNodeCount)) : set.stream().map((targetx) -> this.reconstructPath(targetx.getBestNode(), (BlockPos)map.get(targetx), false)).min(Comparator.comparingDouble(Path::getDistToTarget).thenComparingInt(Path::getNodeCount));
        profilerFiller.pop();
